...
class UserViewLastMsg extends StatelessWidget {
  Widget getMsgWidget(ChatModel chatModel, var unreadMsgCount,
      BuildContext context, var inherited) {
    if (null != chatModel) {
      //print('in getmsg widget  '+chatModel.toString());
      Widget iconType = (chatModel.chatType == ChatModel.IMAGE)
          ? Icon(Icons.photo_camera)
          : Icon(Icons.videocam);
      String txt = (chatModel.chatType == ChatModel.CHAT)
          ? (chatModel != null) ? chatModel.chat : ""
      ...
      return Flex(
        ...
        children: <Widget>[
          ...
          (chatModel.chatType != ChatModel.CHAT)
              ? Flexible(
                  child: Container(
                    ...
                    child: IconTheme(
                      child: iconType,
                      data:
                          IconThemeData(size: 18, color: inherited.otherColor),
                    ),
                  ),
                )
              : Container(
          ...
          getText(txt, unreadMsgCount, context, inherited, chatModel.isD)
        ],
      );
    }
    return Container(
  ...
  Widget getText(var lastMessage, var unreadMsgCount, BuildContext context,
    ...
    return Text(
      ...
      style: isD
          ...
          : TextStyle(fontSize: 15, color: inherited.otherColor),
    );
  }
